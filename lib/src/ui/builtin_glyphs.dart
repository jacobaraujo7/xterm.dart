...
bool paintBuiltinGlyph(
  ...
  if (arms != null) {
    _paintBoxDrawing(canvas, offset, cell, arms, color, _rounded.contains(codepoint));
    return true;
  }
  return false;
}
...
void _paintRoundedCorner(
  ...
  if (u != 0) {
    canvas.drawRect(Rect.fromLTRB(cx - t / 2, o.dy, cx + t / 2, cy - radius), paint);
  }
  if (d != 0) {
    canvas.drawRect(Rect.fromLTRB(cx - t / 2, cy + radius, cx + t / 2, o.dy + h), paint);
  }
  if (l != 0) {
    canvas.drawRect(Rect.fromLTRB(o.dx, cy - t / 2, cx - radius, cy + t / 2), paint);
  }
  if (r != 0) {
    canvas.drawRect(Rect.fromLTRB(cx + radius, cy - t / 2, o.dx + w, cy + t / 2), paint);
  }
